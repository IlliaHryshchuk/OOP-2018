...
void Tournament::setDataOfTournament(std::string day, std::string month, std::string year, std::string place)
{
	system("cls");
	...
	if (infile.is_open())
	{
		...
		infile << day << endl << month << endl << year << endl << place << endl;
		infile.close();
		//setNameOfTournament();
...
void FCteam::addTeams()
{
	...
	if (infile.is_open())
	{
		...
		//{
			...
			cout << "Enter a rating: ";
			cin >> fcteam[i].rating;*/
			infile << this->nameOfTeam << endl << 0 << endl;

			//fcteam.player = new Player[3];
			for (int j = 0; j < 3; j++)
			{
				...
				infile << this->player[j].name << endl << this->player[j].surname << endl << 21 << endl << "Ukraine" << endl;
			}
		//}
		infile.close();
...
Coach* Coach::hireCoach(Coach *maincoach, int n)
{
...
FCnurse* FCnurse::hireNurse(FCnurse *fctopnurse, int n)
{
